...
class Duke(NPC):
    ...
    has_all_quest_app.items(player_app.items)
        Checks if the Player has all app.items to complete the quest
    '''
    ...
    def talk(self, response, player: Player):
        ...
        if response == f"Talk to {self.name}":
            ...
            # Remove app.items used for concoction / potion
    ...
    def has_all_quest_items(self,player_items):
        ...
        Returns
        ----------
        True : if has all app.items to complete quest
        False : if does not have all app.items to complete quest
        '''
        count = 0
        for item in player_items:
